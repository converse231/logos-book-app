...
- **Mascot:** **Fable** the fox (`components/shared/Mascot.tsx`) — a hand-built `react-native-svg` face (ember coat / cream muzzle / ink linework, fixed identity colours), gentle reduced-motion-gated idle bob. Used in empty states (Library, Stats) + onboarding welcome; reusable anywhere via `<Mascot size={n} />`.
