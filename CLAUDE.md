...
- **Texture:** `ScreenBackground` draws a faint blueprint **grid** behind all content — rendered as thin absolutely-positioned **line Views** (40dp cells, 1px, ~8% ink on light / 9% white on dark), NOT a tiled image. (RN's `<Image resizeMode="repeat">` does **not** tile on the New Architecture — it renders a single tile in the corner — so the old `assets/textures/grid.png`+`grain.png` PNG overlays were replaced by the programmatic grid. Those PNGs + `scripts/gen-textures.js` are now unused/legacy.) RN can't do the skill's SVG-filter halftone or CRT scanlines (web-only) and scanlines clash with the light paper, so they're intentionally omitted.
