...
- **B3 — Library, search, reviews.  ✅ DONE (code).** ME: `services/supabase/library.ts` — `getUserBooks/getUserBook/updateBookStatus/updateCurrentPage` (owner-scoped RLS queries with `book:books(*)` embed), `searchBooks/getRecommendedBooks` (client-side Google Books + Open Library fallback in `lib/bookSearch.ts`, no key), `addBook` (re-fetches metadata → `ensure_book` edge fn → upserts `user_books`), `writeReview/getReviews` (RLS; reviewer names via a second `public_profiles` query since you can't embed a view). Composed as `libraryApi` into `liveApi`. **Two notes:** (1) the half-star review UI is **rounded to whole stars** on persist — `reviews.rating` is `smallint(1–5)`; widen to `numeric(2,1)` later if halves must survive. (2) the book-detail **favorite heart is still local-only** — needs a new `setFavorite` LogosApi method (deferred micro-task). **Requires YOU:** `supabase functions deploy ensure_book`.
