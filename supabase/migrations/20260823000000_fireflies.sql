-- Fireflies — the reading currency.
--
-- Additive and non-breaking by construction: a new column with a default, and
-- two new keys on complete_session's result. Every existing caller and every
-- existing key is untouched, so an old client simply ignores the new fields.
--
-- The grant is 3 base, +1 per 10 pages, +1 per 10 minutes, capped at 25.
-- The cap matters: a pouch costs 100, and the economy is tuned so a week of
-- ordinary daily reading earns roughly one. Without a ceiling a single marathon
-- session could out-earn that whole week and the pacing collapses.
--
-- Audiobooks have no page count and print sessions store no minutes, so the
-- time term falls back to wall-clock duration and the page term to zero.
--
-- The dedupe path returns firefliesEarned = 0 on purpose. Offline sessions
-- re-send until acknowledged, and complete_session is idempotent on
-- client_uuid; a non-zero value there would let a queued session appear to pay
-- out twice on the client.

alter table public.users
  add column if not exists fireflies integer not null default 0;

CREATE OR REPLACE FUNCTION public.complete_session(p_client_uuid uuid, p_user_book_id uuid, p_book_id uuid, p_format book_format, p_started_at timestamp with time zone, p_ended_at timestamp with time zone, p_start_page integer, p_end_page integer, p_minutes_listened integer, p_end_position_min integer, p_local_date date, p_source session_source)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  v_fire := least(25, 3
    ...
    + floor(coalesce(v_minutes, v_duration / 60.0) / 10.0)::int);
  ...
  -- Book progress (format-aware); promote want/tbr/dnf â†’ reading; stamp started_at.
  ...
  -- FIX: one-arg form â€” this is the only fn_eval_badges that actually exists.
