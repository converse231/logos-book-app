-- ─────────────────────────────────────────────────────────────────────────────
-- complete_session: the book-progress promotion also treats a TBR book like a
-- want/dnf book — any of them auto-promotes to 'reading' the moment a real
-- session lands against it. (want/tbr/dnf are all "not started"; only status
-- shouldn't be clobbered once a book is 'reading' or 'finished'.)
--
-- Depends on 20260619000000_add_tbr_status.sql having already committed the
-- 'tbr' enum value (run that file first, as its own transaction).
--
...
begin
  ...
  v_badges := public.fn_eval_badges(v_uid, v_session.id);
